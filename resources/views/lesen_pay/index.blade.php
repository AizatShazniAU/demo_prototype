...
<div class="card mb-3 overflow-hidden">
  <div class="card-header">
    ...
    <div class="row align-items-center">
      <div class="col">
        <h4 class="mb-0">PEMBAYARAN LESEN</h4>
      </div>
      <div class="col-auto text-center pe-card">
        <button class="btn btn-secondary me-1 mb-1"><a style="color: black" href="/lesen_pay/create"><i class="fa fa-plus-square"></i>Tambah</a></button>
      </div>
    </div>
  </div>
<br>
...
<table class="buttonsED table table-striped border-bottom" style="text-align: center">
  ...
  @foreach ($lesen_pay as $lesen_pay)
  <tr >
    
    <td><ul> {{ $lesen_pay -> lesen_id}}</ul></td>
    <td><ul> {{ $lesen_pay -> date}}</ul></td>
    <td><ul> {{ $lesen_pay -> payMet}}</ul></td>
    <td><ul> {{ $lesen_pay -> status}}</ul></td>
    <td>
      <button class="buttonsED btn btn-sm btn-falcon-default" ><a style="color: black" href="/lesen_pay/{{$lesen_pay->id}}/edit"><i class="fas fa-pen" aria-hidden="true"></i></a></button>
      {{-- <a href="/lesen_pay/{{$lesen_pay->id}}/edit">KEMASKINI</a> --}}
      <form action="/lesen_pay/{{ $lesen_pay -> id }}" method="POST">
        @csrf
        @method('DELETE')
        <button type="submit" class="buttonsED btn btn-sm btn-falcon-default" style="color: black"><i class="fa fa-trash" aria-hidden="true"></i></button>
        </form>
    </td>
    </tr>
@endforeach
...
<br>
{{-- <h2>PEMBAYARAN LESEN</h2> --}}



<br>
{{-- <a href="/lesen_pay/create">Tambah</a> --}}
