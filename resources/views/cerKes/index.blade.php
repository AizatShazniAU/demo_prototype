...
<table class="table table-striped border-bottom" style="text-align: center">
  ...
  @foreach ($cerKes as $cerKes)
  <tr >
    
    <td><ul> {{ $cerKes -> speaker}}</ul></td>
    <td><ul> {{ $cerKes -> date}}</ul></td>
    <td><ul> {{ $cerKes -> time}}</ul></td>
    <td><ul> {{ $cerKes -> loc}}</ul></td>
    @can('isAdmin')
    <td><a href="/cerKes/{{ $cerKes -> id }}/edit">KEMASKINI</a>
      <form action="/cerKes/{{ $cerKes -> id }}" method="POST">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-sm btn-falcon-default">
          <i class="fa fa-trash" aria-hidden="true"></i>BUANG</button>
        </form>
    </td>
    @else
    @endcan
